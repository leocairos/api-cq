-- public.vw_15dforno_samples source

CREATE OR REPLACE VIEW public.vw_15dforno_samples
AS SELECT s.id,
    s.identification,
    s.service_center_id,
    s.created_at,
  ...
  WHERE s.current_status_edition_date_time >= (now() - '15 days'::interval) AND (s.sample_collection_point_id = ANY (ARRAY[1023, 1019, 1675, 1736]));
